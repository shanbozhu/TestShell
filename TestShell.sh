...
# 7、合并（将列表或其他序列合并成字符串）
# 8、去空格和特定字符（去除字符串前后或中间的空格或特定字符）
# 9、大小写转换（转换字符串的大小写）
# 10、对齐（调整字符串的显示对齐方式）
# 11、格式化（格式化字符串内容）
# 12、编码与解码（将字符串编码为字节或从字节解码为字符串）
# 13、字符计数（统计某字符或子字符串出现的次数）
# 14、判断性质（判断字符串是否满足某些条件。如前缀、后缀等）
# 15、翻转（反转字符串内容）

## 四、数据运算

# (()) 对整型进行运算。对括号内的变量取值时，$可省略
# (()) 对整型进行关系运算时使用>、<、==等符号。
# [[]] 对整型或字符串进行关系运算。对整型进行关系运算时使用-eq等选项，对字符串进行关系运算时使用>、<、==等符号。[[]]必须结合判断或循环使用，无法单独使用。
